...
class WhisperAPI {
            ...
            try {
                ...
                if (status.error) {
                    ...
                    if (status.needsReconnect) {
                        ...
                        if (reconnectSuccess) {
                            console.log('재연결 성공 - 진행률 모니터링 계속');
                            return; // 다음 주기에서 다시 시도
                        } else {
                            console.log('재연결 실패 - 진행률 모니터링 중단');
                        }
                    }
                ...
                if (status.status === 'completed') {
                    ...
                    this.stopProgressMonitoring();
                    onError(status.message || '작업 처리 중 오류가 발생했습니다.');
                }
            } catch (error) {
    ...
    async attemptAutoReconnect(reason = '연결 끊김 감지') {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (this.pendingTask) {
                    console.log('🔄 대기 중인 작업 자동 재시작');
                    setTimeout(() => this.resumePendingTask(), 1000); // 1초 대기 후 재시작
                }
    ...
    async resumePendingTask() {
        ...
        this.pendingTask = null; // 대기열에서 제거

        console.log('🚀 대기 중인 작업 재시작:', task.file.name);
        ...
        try {
            ...
            if (result.success) {
                ...
                // 재시작 실패
                task.onError(result.message);
            }
        } catch (error) {
            console.error('작업 재시작 오류:', error);
            task.onError('작업 재시작 중 오류가 발생했습니다.');
        }
    }
...
class WhisperGeneratorWithAPI {
    ...
    onProgressUpdate(status) {
        ...
        console.log(`[${currentTime}] 진행률 업데이트:`, status);
        
        // 비동기 UI 업데이트로 블로킹 방지
        requestAnimationFrame(() => {
            if (this.elements.progressFill) {
                this.elements.progressFill.style.width = `${status.progress || 0}%`;
            }
